...
function OperatorMenu({ name, initials, onLogout }: { name: string; initials: string; onLogout: () => void }) {
  ...
  return (
    <div className="relative flex items-center gap-2" ref={menuRef}>
      ...
      {open && (
        <div className="absolute right-0 top-10 w-48 bg-[#111] border border-[#1a1a1a] rounded-lg shadow-xl z-50 py-1 font-mono text-xs">
          <button
            className="w-full text-left px-4 py-2.5 text-gray-300 hover:bg-white/5 hover:text-soterion-accent transition-colors flex items-center gap-2"
            onClick={() => { setOpen(false); navigate("/app/shift-handoff"); }}
          >
          ...
          <button
            className="w-full text-left px-4 py-2.5 text-gray-400 hover:bg-white/5 hover:text-[#ef4444] transition-colors flex items-center gap-2"
  ...
  return (
    <header className="h-14 bg-soterion-surface border-b border-soterion-border flex items-center justify-between px-6">
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Operator name + avatar with dropdown */}
        <OperatorMenu name={operator?.name ?? ""} initials={initials} onLogout={logout} />
      </div>
    </header>
  );
}
